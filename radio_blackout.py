...
Then convert to XML format for CCMC (http://ccmc.gsfc.nasa.gov/challenges/flare.php)
Then doftp to ftp://hanna.ccmc.gsfc.nasa.gov/pub/FlareScoreboard/in/MO_TOT1/ using cron job

Notes:
MASS files are in form YYYY-MM-DDThh-mm-ss.json

***Comment out 'put' line in cron job or the ftp part below for testing purposes if you dont want it sent to CCMC***
'''
...
def main():
    """Determine if its day or night, then run correct code."""
    ##Get all of todays and yesterdays data
    grab_data()
    ##Now find latest file
    names = [fn for fn in os.listdir(FOLDER) if any([fn.endswith('json')])]
    sorted_file = sorted(names, key=lambda x: datetime.datetime.strptime(x.strip()[0:19], '%Y-%m-%dT%H-%M-%S'))
    ...
    if (issued_time == previous_issued_time) == True:
        ...
        ##Grab probabilities
        m_probs = flare_forecast["probabilities"][0] #day1,day2,day3,day4
        ...
        for i in range(0, len(m_probs)):
            ...
            xml_file = xml(issued_time, start_time, end_time, m_prob, x_prob, day)
            print "The following file will be ftp-ed to CCMC:"
            print xml_file
            #ftp(xml_file)
            ##Some housekeeping at the to remove temporary files
        clean_up(newest, xml_file=None, keep=sorted_file[len(sorted_file)-1])#, xml_file)
    print "All done!"
...
def grab_data():
    """Grabbing whole day for now - too complicated otherwise with the varying times the forecasters issue it
